...
def main():
    moeda_origem = input("Digite uma moeda ORIGEM para cambiar:").upper()
    URL = URL_BASE + moeda_origem
    resposta = requests.get(URL)

    if resposta.status_code == 200:
        cambio = resposta.json()
        moeda_destino = input("Digite a moeda destino:")

        valor_cambio = cambio['rates'][moeda_destino]
        print(f'Cambio do {moeda_origem} para {moeda_destino}: 1 {moeda_origem} = {valor_cambio} {moeda_destino}.')
